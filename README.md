...
如果要使用 OpenRouter（例如 Kimi K2.6）：

```bash
pip3 install -e .
export OPENROUTER_API_KEY=your_openrouter_key
python3 -m ckb_talk_radar --hours 24 --model moonshotai/kimi-k2.6
```
...
3. 可选：在 `Settings -> Secrets and variables -> Actions` 中添加 `OPENAI_API_KEY`、`OPENROUTER_API_KEY` 或 `MOONSHOT_API_KEY`。
